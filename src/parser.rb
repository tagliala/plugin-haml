...
module Haml
  class Parser
    class ParseNode
      ESCAPE = /Haml::Helpers.html_escape\(\((.+)\)\)/
      ...
      # escaped.
      special_chars = Haml::Parser::SPECIAL_CHARACTERS.map { |char| Regexp.escape(char) }

      SPECIAL_START = /\A(?:#{special_chars.join('|')})/

      def as_json # rubocop:disable Metrics/AbcSize,Metrics/CyclomaticComplexity,Metrics/MethodLength,Metrics/PerceivedComplexity
        case type
        ...
        when :script
          to_h.tap do |json|
            ...
            if json[:value][:text].match?(ESCAPE)
              json[:value][:text].gsub!(ESCAPE) { ::Regexp.last_match(1) }
